...
namespace Niantic.ARDK.Extensions
{
    ...
    ARSessionListener
  {
    ...
    private PlaneDetection _detectedPlaneTypes;
        ...
        public static ARPlaneManager instant;
    ...
    private PlaneDetection _prevDetectedPlaneTypes;
     ...
     public Dictionary<Guid,GameObject> temparraylist = new Dictionary<Guid,GameObject>();
    ...
    private void CreateAnchorPrefab(IARPlaneAnchor anchor)
    {
      ...
      if (_planePrefab == null)
        return;
          ...
          //  Debug.Log(anchor.Alignment);
            //Horizontal
          //  if (anchor.Alignment.Equals("Vertical")) {
                ...
                temparraylist.Add(anchor.Identifier,plane);
            // }
    ...
    public void RemoveAnchor(IARAnchor anchor)
    {
      ...
      _planeLookup.Remove(anchor.Identifier);
       temparraylist.Remove(anchor.Identifier);
    }
    ...
    private void RefreshAnchor(IARPlaneAnchor anchor)
    {
      if (_planeLookup.TryGetValue(anchor.Identifier, out GameObject go))
      {
            ...
            //Debug.Log(anchor.Transform.ToPosition()+ "center" + anchor.Center);
        ...
        // ARCore plane anchors will update the transform
                ...
                go.transform.localScale = new Vector3(anchor.Extent.x,0.1f,anchor.Extent.z);  
                go.GetComponent<PlanefindingGrid>().prevscale= new Vector3(anchor.Extent.x, 0.1f, anchor.Extent.z);
                go.GetComponent<PlanefindingGrid>().tempAnchor = anchor;
                txttext.text = go.transform.position.ToString();

                if (anchor.Alignment.ToString().Equals("Vertical"))
                {
                    Setmaterial(go);
                    ...
                    txttext.text = go.transform.position.y.ToString();

                    //  go.transform.localScale = Vector3.one;
                    //  go.transform.GetChild(0).transform.localScale = new Vector3(0.08f, 2f,0.08f);                
        ...
        {
           // Debug.Log(obj.name);
            var PlaneMaterial = obj.transform.GetChild(0).gameObject.GetComponent<MeshRenderer>().material;            
            PlaneMaterial.SetColor("_TexTintColor", Color.red);
